...
def get_recent_logs(
    ...
    try:
        ...
        is_premium = current_user.plan and current_user.plan.lower() not in ["free", "starter"]
...
def create_tracked_signal(
    ...
    try:
        ...
        new_sig = SignalDB(
            ...
            confidence=float(signal_data.get("confidence")) if signal_data.get("confidence") is not None else 0.0,
...
def toggle_save_signal(
    ...
    try:
        ...
        if sig.user_id == current_user.id:
            ...
            db.commit()
            return {"status": "success", "id": sig.id, "is_saved": new_state, "action": "updated"}
            ...
            # Too complex. For this iteration, if they click Track on a system signal, we clone it.
            ...
            new_sig = Signal(
                ...
                extra=sig.extra # Is extra stored in JSON? Signal model in models_db doesn't show Extra column in finding above.
                # models_db definition of Signal:
                # strategy_id, mode, etc. NO EXTRA column in the snippet I saw earlier (lines 1-36).
                # But Unified Schema has extra. 
                # Let's rely on what's in DB.
            )
            db.add(new_sig)
            db.commit()
            return {"status": "success", "id": new_sig.id, "is_saved": 1, "action": "cloned"}
...
def notify_telegram_endpoint(payload: TelegramPayload, current_user: User = Depends(get_current_user)):
