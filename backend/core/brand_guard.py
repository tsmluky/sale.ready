...
BANNED_WORDS = [
    "deepseek", "llm", "large language model", "modelo de lenguaje", 
    "openai", "chatgpt", "anthropic", "claude", "as an ai", "como una ia",
    "soy un modelo", "i am an ai"
]

# Regex patterns for minimum viability checks in Setup context
PATTERN_INVALIDATION = r"(invalidaci|anula|cancel|stop|sl|pierde|rompe|por debajo de|cierre|invalidates)"
PATTERN_PLAN = r"(plan|haría|sugiero|recomiendo|esperar|busco|entrar|target|objetivo|zona)"
...
def detect_intent(user_msg: str) -> str:
    ...
    setup_keywords = [
        "entry", "entrada", "entro", "compro", "vendo", "long", "short", 
        "setup", "trade", "operación", "stop", "tp", "sl", "target"
    ]
...
def repair_response(original_text: str, violations: List[str], system_instruction: str) -> str:
    """
